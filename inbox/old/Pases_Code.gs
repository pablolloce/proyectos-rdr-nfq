...
function enviarCorreoSeguro(ctx, asunto, cuerpo, fechaStr, fila, esInicio) {
  ...
  try {
    const threads = GmailApp.search('subject:"' + asuntoHilo + '"', 0, 1);
    const thread = threads.length > 0 ? threads[0] : null;
    if (thread) {
      thread.replyAll(plainText, { htmlBody, name: "RDR Hub" });
    } else if (esInicio) {
      const msg = GmailApp.createDraft(dest, asuntoHilo, plainText, { htmlBody, name: "RDR Hub" }).send();
      Utilities.sleep(1500);
      sheet.getRange(fila, 12).setValue(msg.getThread().getId());
    }
  } catch (e) {
    console.error(e);
  }
