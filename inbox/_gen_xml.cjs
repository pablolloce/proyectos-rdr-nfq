...
// ── Quiz · 20 preguntas ────────────────────────────────────────────────────
const QUES = [
  {q:"¿Qué es XML?",opts:["Un lenguaje de marcado que estructura datos en árbol de nodos (el contenedor de los datos)","Un motor de base de datos","Un lenguaje de programación procedural","Un protocolo de red"],c:0,exp:"XML estructura la información en forma de árbol de etiquetas. Responde a '¿qué información tenemos?': es el contenedor de los datos."},
  {q:"¿Cuál es la función principal de un XSD?",opts:["Transformar XML a otro formato","Definir la estructura y las reglas, y validar el XML","Navegar entre nodos","Cifrar el mensaje"],c:1,exp:"XSD (XML Schema Definition) define la gramática/reglas y valida que el XML las cumpla. Responde: '¿cumple este archivo las reglas?'."},
  {q:"¿Para qué sirve XPath?",opts:["Validar tipos de datos","Transformar a HTML","Navegar y seleccionar fragmentos (nodos) de un XML","Comprimir el archivo"],c:2,exp:"XPath es el lenguaje de consultas para navegar el árbol y seleccionar nodos. Responde: '¿dónde está el dato que busco?'."},
  {q:"¿Qué hace XSLT?",opts:["Transforma un XML en otro XML con un sistema de etiquetas distinto","Define el esquema de validación","Almacena los datos en disco","Enruta paquetes de red"],c:0,exp:"XSLT transforma un XML (con cierto XSD) en otro XML con otro etiquetado — el que necesita el sistema destino. Es la clave de la interoperabilidad."},
  {q:"¿Qué son los workflows en RDR?",opts:["Esquemas de validación XSD","Pequeños programas integrados que reciben mensajes, aplican lógica (ifs, bucles) y, con XPath, deciden acciones sobre la BBDD","Ficheros de definición de pantallas","Hojas de transformación de formato"],c:1,exp:"Los workflows son pequeños programas dentro de la aplicación: reciben mensajes, introducen lógica (ifs, bucles) y usan mucho XPath para extraer datos de una petición/ACK y decidir las acciones sobre la BBDD según el valor."},
  {q:"¿Con qué se generan los mensajes XML a partir de los datos de Oracle en RDR?",opts:["Con XSLT directamente","Con funciones SQL/XML como XMLELEMENT y XMLAGG en las queries y extracciones","Con un XSD","A mano, uno a uno"],c:1,exp:"Las queries (.sql) y las Extracciones Genéricas usan funciones SQL/XML como XMLELEMENT y XMLAGG para extraer los datos de Oracle (FT_T_*) y formar los mensajes XML."},
  {q:"¿Qué selecciona la expresión XPath /catalogo/libro[precio < 20]?",opts:["Todos los libros del catálogo","Los libros cuyo precio es menor que 20","El primer libro","El atributo precio"],c:1,exp:"El predicado [precio < 20] filtra: devuelve solo los libros que cuestan menos de 20."},
  {q:"¿Qué busca //usuario[@id='123']?",opts:["El usuario cuyo atributo id es '123'","Todos los usuarios","El elemento hijo 123","La ruta absoluta /usuario/123"],c:0,exp:"@id apunta a un atributo: selecciona el usuario con atributo id igual a '123'."},
  {q:"¿De qué tecnología depende XSLT para apuntar a los datos?",opts:["XSD","XPath","JSON","SQL"],c:1,exp:"XSLT usa expresiones XPath para localizar los datos del XML original que va a transformar o reordenar."},
  {q:"Si un XML de factura no incluye la etiqueta <total>, el XSD…",opts:["La crea automáticamente","Rechaza el archivo en la validación","La ignora","La convierte a JSON"],c:1,exp:"El validador XSD rechaza el archivo inmediatamente, evitando que el sistema falle más adelante."},
  {q:"En RDR, ¿qué da servicio Online (publicación y petición/respuesta) y qué da servicio Batch (extracción masiva)?",opts:["Online: Extracciones Genéricas · Batch: queries","Online: queries de GoldenSource · Batch: Extracciones Genéricas","Ambos usan las mismas queries .gsp","Online: FiXML · Batch: JSON"],c:1,exp:"Las queries de GoldenSource dan servicio Online; las Extracciones Genéricas dan servicio Batch. En ambos casos la salida es XML, que luego se transforma si hace falta (Online a FiXML, Batch al formato de cada consumidor)."},
  {q:"¿Cómo se definen las pantallas (SpeedScreens) y la lógica (Models) de Workstation?",opts:["En XML","En SQL","En Java compilado","En JSON"],c:0,exp:"Las pantallas (SpeedScreens) y los Models se definen íntegramente en archivos XML."},
  {q:"¿Qué formato XML usa la mensajería del bus corporativo (ESB KYRS)?",opts:["SOAP","FiXML (estándar XML para mercados financieros)","HTML","CSV"],c:1,exp:"La integración con el ESB (UUAA KYRS) usa mensajes FiXML, que viajan encriptados."},
  {q:"¿Para qué se usa principalmente XSLT en RDR?",opts:["Para validar la entrada de los mensajes","Para transformar los datos maestros a los formatos de los consumidores (ABACO, Murex, Calypso)","Para almacenar datos en Oracle","Para definir las pantallas"],c:1,exp:"XSLT adapta los datos maestros de GoldenSource a los formatos requeridos por cada consumidor."},
  {q:"¿Qué relación tienen los mensajes XML de RDR con los XSD?",opts:["Solo las extracciones batch usan XSD","Cada XML que se mueve en RDR tiene un XSD asociado que valida su formato (petición, respuesta, publicación, extracción)","Los XSD solo se usan en desarrollo","El XSD sustituye al XSLT"],c:1,exp:"Cada XML —cada petición, respuesta, publicación y extracción— tiene su XSD asociado: es la forma de entenderse con los consumidores. Sin esa validación común sería un caos."},
  {q:"Dentro de los model_*.xml, ¿para qué se usa XPath?",opts:["Para definir reglas de validación sobre los campos de las pantallas","Para transformar a HTML","Para conectar con Oracle","Para cifrar datos"],c:0,exp:"En los Models, XPath define reglas de validación complejas sobre los campos de las pantallas."},
  {q:"¿Qué tipo de operativa da el servicio Online de RDR?",opts:["Publicación y petición/respuesta en tiempo real (mensajes al ESB KYRS en FiXML)","Extracción masiva nocturna en ficheros","Copias de seguridad de la BBDD","Compilación de los Models"],c:0,exp:"El servicio Online da publicación y petición/respuesta en tiempo real; los mensajes salen al bus ESB KYRS en FiXML encriptado — crítico para trading y riesgos."},
  {q:"En el servicio Batch, ¿cómo se distribuyen las extracciones masivas a los consumidores?",opts:["Vía DataX / SFTP","En tiempo real por el ESB","Por API REST síncrona","No se distribuyen; se consultan online"],c:0,exp:"Las Extracciones Genéricas (p. ej. ThirdParties.xml) se generan en XML y se distribuyen a los +40 consumidores vía DataX / SFTP."},
  {q:"En ambos servicios (Online y Batch), ¿en qué formato se saca primero la información de la BBDD?",opts:["Directamente en el formato final de cada consumidor","En XML, que luego se transforma si hace falta","En binario comprimido","En texto plano sin estructura"],c:1,exp:"En ambos casos la información se saca primero en XML; luego se transforma si hace falta — Online a FiXML, Batch al formato de cada consumidor."},
  {q:"¿Con cuántos sistemas consumidores se comunica RDR mediante estas tecnologías?",opts:["Menos de 5","Alrededor de 10","Más de 40","Exactamente 2"],c:2,exp:"XML/XSLT/XSD/XPath permiten que el repositorio maestro se comunique con más de 40 sistemas consumidores."}
];
