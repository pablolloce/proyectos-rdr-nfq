...
const CARD =
  "rdr-rise group relative flex items-center gap-3 overflow-hidden rounded-xl border p-3 backdrop-blur-md transition duration-200 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-serene focus-visible:ring-offset-2 focus-visible:ring-offset-midnight";
...
function CardInner({ item, accent }) {
  ...
  return (
    <>
      <span aria-hidden className="rdr-spot" style={{ background: `radial-gradient(120px circle at var(--mx,50%) var(--my,50%), ${rgba(accent, 0.22)}, transparent 70%)` }} />
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border" style={{ borderColor: rgba(accent, 0.3), background: rgba(accent, 0.12), color: accent }}>
        <Icon size={18} />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-bold text-sand">{item.label}</span>
        {item.desc && <span className="block truncate text-xs text-sand/60">{item.desc}</span>}
      </span>
      <Hint size={15} className="shrink-0 text-sand/40 transition-all duration-200 group-hover:translate-x-0.5 group-hover:text-sand/85" />
    </>
  );
}
...
function Card({ item, accent, delay }) {
  ...
  if (item.actions) {
    const { icon: Icon } = item;
    return (
      <div className={`rdr-rise relative flex flex-col gap-2.5 rounded-xl border border-white/10 bg-white/[0.055] p-3 backdrop-blur-md`} style={style}>
...
function FeatureCard({ item, accent, delay }) {
  return (
    <Link
      ...
      className="rdr-rise group relative flex flex-col gap-2 overflow-hidden rounded-2xl border border-serene/30 bg-gradient-to-br from-white/[0.1] to-white/[0.03] p-4 backdrop-blur-md transition duration-200 hover:-translate-y-0.5 hover:border-serene/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-serene focus-visible:ring-offset-2 focus-visible:ring-offset-midnight"
      style={{ animationDelay: `${delay}ms` }}
    >
      <span aria-hidden className="pointer-events-none absolute -right-5 -top-5 text-serene/15 transition-transform duration-500 group-hover:rotate-12"><IconOrbit size={120} /></span>
      <span className="grid h-11 w-11 place-items-center rounded-xl border border-serene/40 text-serene" style={{ background: "linear-gradient(145deg, rgba(133,200,255,.25), rgba(133,200,255,.06))" }}><IconGrad size={24} /></span>
      <div className="relative">
        <span className="block font-display text-lg font-bold text-sand">{item.label}</span>
        <span className="block text-xs text-sand/70">{item.desc}</span>
      </div>
      <span className="relative mt-1 flex items-center gap-1.5 text-xs font-bold text-serene">Explorar la ruta <IconArrow size={15} className="transition-transform group-hover:translate-x-1" /></span>
    </Link>
  );
}
...
function TypeColumn({ sec, delay }) {
  return (
    <section aria-labelledby={`col-${sec.id}`} className="flex flex-col self-start rounded-2xl border border-white/[0.07] bg-white/[0.02] p-3 backdrop-blur-sm" style={{ boxShadow: `inset 0 2px 0 ${rgba(sec.color, 0.5)}` }}>
      <div className="mb-3 flex items-center gap-2 px-1">
        <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: sec.color }} />
        <span className="font-display text-[11px] font-bold tabular-nums" style={{ color: sec.color }}>{sec.n}</span>
...
export default function BentoHub() {
  ...
  return (
    <main className="relative min-h-dvh w-full">
      <AmbientBackground />
      <div className="mx-auto w-full max-w-7xl px-5 pb-12 pt-24 sm:px-6">
        {/* Columnas por tipo (el título va en el header) */}
        <div className={`grid grid-cols-1 items-start gap-4 sm:grid-cols-2 ${gridCols}`}>
