      ...
      {sheet && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-[71] mx-auto flex max-h-[70vh] w-full max-w-lg flex-col rounded-t-3xl border border-b-0 border-white/15 bg-[#0b1450] pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[0_-8px_30px_rgba(0,0,0,0.4)]"
          >
              ...
              {sheet.ausencias.map((a, i) => {
                ...
                return (
                  <div key={`${a.nombre}-${i}`} className="flex items-center gap-3 border-b border-white/[0.07] py-3 last:border-b-0">
                    <span className="h-3 w-3 shrink-0 rounded-full ring-2 ring-white/15" style={{ backgroundColor: emp?.color || PALETTE.serene }} aria-hidden />
